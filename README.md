...
분석 데이터 전체에 적용한 결과값 accuracy의 값이 0.84이다. 정확성이 84% 정도로 학습결과가 나왔다 정확도가 높은 편은 아닌것 같다.

### 3-3.배운점&느낀점
사람들은 리뷰를 남길때 대부분 긍정적으로 쓴다는걸 느꼈다. 리뷰의 영향이 대단하다고 느꼈다. 내가 남긴 리뷰가 한 두개씩 모여 처음 구매하는 사람한테 또다른 정보를 줄 수 있다는것을 느겼다. 아쉬운 점은 텍스트 데이터가 영어라 하나 하나 번역해가면서 보기에는 효율이 좋지 못했다는 점이다. 다음 학기에 실습할 한글 텍스트 데이터를 가지고 학습하면 보기 편해 실습하는데 도움이 될 것 같다. 마지막으로 내가 만든 프로젝트를 기업 같은곳에서 좀더 다듬고 발전시키면 물건을 구매한는 소비자나 판매하는 판매자 또한 앱을 관리하는 관리자 등 많은 사람들에게 도움이 될 것 같다.
